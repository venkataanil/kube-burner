...
func runCreateOrIncremental(ctx context.Context, jobExecutor JobExecutor, measurementsFactory *measurements.MeasurementsFactory, kubeClientProvider *config.KubeClientProvider, embedCfg *fileutils.EmbedConfiguration, measurementsJobName string, metricsScraper metrics.Scraper, configSpec config.Spec) ([]error, []prometheus.Job) {
	...
	// RunCreateJob handles staging internally - objects default to stage 0
...
func (ex *JobExecutor) gc(ctx context.Context, wg *sync.WaitGroup) {
	...
	// Clean up objects in reverse stage order (highest stage first)
	// When no staging is configured, all objects are in stage 0
	ex.gcStaged(ctx, labelSelector)
	// Clean up namespaces after objects
	ex.gcNamespaces(ctx, labelSelector)
}

// gcStaged cleans up objects in reverse stage order (highest stage first)
func (ex *JobExecutor) gcStaged(ctx context.Context, labelSelector string) {
	stageGroups := ex.groupObjectsByStage()

	// Sort stages in reverse order (highest first)
	sort.Slice(stageGroups, func(i, j int) bool {
		return stageGroups[i].number > stageGroups[j].number
	})

	for _, stage := range stageGroups {
		log.Infof("Cleaning up stage %d objects", stage.number)
		for _, obj := range stage.objects {
			// Skip objects that were already cleaned up during staged execution
			if obj.Stage.Cleanup {
				log.Debugf("Skipping %s cleanup (already cleaned during staged execution)", obj.gvr.Resource)
				continue
			}
			if !obj.namespaced {
...
// gcNamespaces cleans up namespaces created by the job
func (ex *JobExecutor) gcNamespaces(ctx context.Context, labelSelector string) {
